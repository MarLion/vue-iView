  ...
  <div class="banner-container content-pad">
    <div class="configure-fun">
      <div class="community-search">
        ...
        <span>
            <Select v-model="listParams.status" @on-change="getList" style="width:200px">
    ...
    >
      ...
      <Form :model="bannerFormData" ref="bannerForm" :rules="ruleValidate" :label-width="80" class="mt20">
        ...
        <FormItem label="活动类型：">
          <Select v-model="bannerFormData.type">
            <Option v-for="item in typeList" :value="item.id" :key="item.name">{{item.name}}</Option>
          </Select>
        </FormItem>
        <FormItem label="关联活动：" prop="contentName">
          <AutoComplete
            v-model="bannerFormData.contentName"
            icon="ios-search"
            placeholder="输入活动名查询"
            :disabled="isForbidden"
            @on-search="searchArticle(bannerFormData.contentName,bannerFormData.type)"
          >
    ...
    >
      ...
      <Form :model="bannerReFormData" ref="bannerReForm" :rules="ruleValidate" :label-width="80" class="mt20">
        ...
        <FormItem label="活动类型：">
          <Select v-model="bannerReFormData.type">
            <Option v-for="item in typeList" :value="item.id" :key="item.name">{{item.name}}</Option>
          </Select>
        </FormItem>
        <FormItem label="关联活动：" prop="contentName">
          <AutoComplete
            ...
            @on-search="searchReArticle(bannerReFormData.contentName,bannerReFormData.type)"
          >
...
<script>
  ...
  export default {
          ...
          },
          {
            ...
            render:(h,params) => {
              const revise =   h('Button',{
                ...
                on:{
                  click:() => {
                    ...
                    this.reClick(params.row.id,params.row.type,params.row.contentName,params.row.contentId,params.row.picture,params.row.name,params.row.showType.toString());
                  }
                }
              },'修改');
              const recommend = h('Button',{
                ...
                on:{
                  click:() => {
                    axios.TvBannerStick({id:params.row.id,showType:1})
                      .then(res => {
                        console.log(res);
                        if (res.code === '0') {
                          this.bannerTip = true;
                          ...
                          this.bannerTip = true;
                          this.$refs.bannerTip.innerHTML = res.message;
                        }
                      })
                      .catch(error => {
                        console.log(error);
              ...
              const noRecommend = h('Button',{
                ...
                on:{
                  click:() => {
                    axios.TvBannerStick({id:params.row.id,showType:0})
                      .then(res => {
                        //console.log(res);
                        if (res.code === '0') {
                          this.bannerTip = true;
              ...
              const show =  h('Button',{
                ...
                on:{
                  click:() => {
                    axios.TvBannerShow({id:params.row.id,status:1})
                      .then(res => {
                        //console.log(res);
                        if (res.code === '0') {
                          this.bannerTip = true;
              ...
              const hidden = h('Button',{
                ...
                on:{
                  click:() => {
                    axios.TvBannerShow({id:params.row.id,status:0})
                      .then(res => {
                        //console.log(res);
                        if (res.code === '0') {
                          this.bannerTip = true;
    ...
    methods:{
      getList:function () {
        axios.TvBannerList(this.listParams)
          .then(res => {
            //console.log(res);
            if (res.code === 0) {
      ...
      //查询活动
      searchArticle:function (name,type) {
        axios.TvActiCheck({name:name,type:type})
          .then(res => {
            //console.log(res);
            if (res.code === '0') {
      ...
      bannerSub:function () {
        this.$refs.bannerForm.validate(valid => {
          if (valid) {
            axios.TvBannerAdd(this.bannerFormData)
              .then(res => {
                //console.log(res);
                if (res.code === '0') {
      ...
      //修改
      reClick:function (id,type,contentName,contentId,picture,name,showType) {
        ...
        this.bannerReFormData.type = type;
      ...
      searchReArticle:function(name,type){
        axios.TvActiCheck({name:name,type:type})
          .then(res => {
            //console.log(res);
            if (res.code === '0') {
      ...
      bannerReSub:function () {
        this.$refs.bannerReForm.validate(valid => {
          if (valid) {
            axios.TvBannerAdd(this.bannerReFormData)
              .then(res => {
                //console.log(res);
                if (res.code === '0') {
